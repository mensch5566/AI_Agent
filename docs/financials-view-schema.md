...
- **period_kind**：`quarter_duration` / `fy_annual_duration` / `ytd_duration` / `instant_period_end` / `derived_q4` / `ttm_duration`（後二者 metrics-only；`ttm_duration` = EL2 quarterly 滾動 12 月比率 ROE/ROA）
