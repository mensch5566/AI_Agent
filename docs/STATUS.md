...
- **derive-analytics expansion — Phase 0 (MVP contract hardening) shipped + deployed** (2026-05-31). Roadmap in `docs/derive-analytics-expansion-plan.md` (Codex-reviewed ×2). Before adding any new analyst metrics, the existing 5-ratio MVP was hardened to the same metrics contract as derive-base:
  ...
  - **0.5 Freshness gate**: `verify_derived_freshness()` gained a `required_keys` param. The analytics `--apply` gate fails closed before `apply(batch)` on every unsafe state — incomplete run (exit 2), missing run when Supabase already has analytics rows (exit 4 unless `--allow-missing-derived`), and stale inputs (exit 3). `analytics_required_keys(derive_status)` additionally requires `derive_base` lineage whenever derive-base is loaded, so an analytics run produced before derive-base existed can't ship raw-only ratios beside fresh derive-base metrics. `client` is lazily obtained in the apply block (fixes an UnboundLocalError on the parse-only + analytics-loaded path).
  - Deployed: prototype synced to CC_Switch_Config (`974af9a` + `85df8f8`) + 3 runtimes; backend in AI_Agent (`bf6d246` + Codex-review fixes `8bec484` / `22d4a82`). All 4 tickers (LITE/AAOI/INTC/SNDK) re-run + re-upserted to production; verified 0 YTD orphans / 0 legacy cell_id / 0 missing formula / derive_base lineage present. Tests: scripts 267 + prototype 21, 0 regression. Two Codex functional-review rounds closed (5 + 1 findings).
