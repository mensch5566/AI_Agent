#!/usr/bin/env python3
"""
台股 Inline XBRL (iXBRL) 解析腳本
從下載的 .html 檔案中提取財務數據，與 Supabase 比對
"""
...
METRIC_MAPPING = {
    ...
    '資產總計': 'total_assets',  # 改為 資產總計（台股用此）
    'Total assets': 'total_assets',

    '流動負債合計': 'total_current_liabilities',
    'Total current liabilities': 'total_current_liabilities',

    '負債總計': 'total_liabilities',  # 改為 負債總計
    'Total liabilities': 'total_liabilities',

    '權益總額': 'total_equity',  # 優先此項（包含非控制權益）
    'Total equity': 'total_equity',
    'Equity': 'total_equity',  # 備用
}
...
def parse_ixbrl_html(file_path: str) -> dict:
    ...
    # 簡易提取公司 ID（從檔名或 HTML title）
    company_info = {
        'ticker': '2454',  # 硬編碼，也可從檔名提取
        'period': 'FY2025_Q4'
    }
    ...
    # 先蒐集所有候選，然後按優先級選擇
    metric_candidates = {}  # {metric_key: [(score, tw_name, value, table_idx, row_idx), ...]}
...
def compare_with_supabase(ticker: str, parsed_data: dict):
    ...
    for metric, value in parsed_data['metrics'].items():
        ...
        # 比較（允許單位差異）
        # 台股 XBRL 單位通常是基本單位，Supabase 可能以千為單位
...
def main():
    # 指定 iXBRL 檔案
    file_path = "/Users/mensch5566/Downloads/tifrs-fr1-m1-ci-cr-2454-2025Q4.html"
    ...
    # 與 Supabase 比對
    compare_with_supabase(parsed['info'].get('ticker', '2454'), parsed)
